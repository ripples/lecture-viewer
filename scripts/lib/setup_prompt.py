...
def request_input(text, env=None, password=False, warning=None, display_default=True):
...
def setup():
    def setup_mailer():
        if not (request_input("Enter a gmail mailer username: ", env="MAILER_USER") or
                    request_input("Enter a gmail mailer password: ", env="MAILER_PASSWORD", password=True)):
            print("WARNING: both a mailer username and password is needed to send emails")

    def setup_db():
        request_input("Enter password for the database", env="MYSQL_ROOT_PASSWORD", password=True)

    def setup_media_dir():
        host_media_dir = Path(request_input(
    ...
    def setup_media_files(host_media_dir):
        school_logo = host_media_dir.joinpath(Path(request_input(
        ...
        users_csv_path = Path(request_input(
    ...
    def setup_miscellaneous():
        envs["NODE_ENV"] = "production" if prompt("Is this a production environment?") else defaults["NODE_ENV"]
        request_input("Enter a custom signing key (not suggested). Defaults to 64 character random string: ",
                      env="SIGNING_KEY", display_default=False)
        request_input("Enter the host url for the Lecture Viewer.", env="CLIENT_BASE_URL")
